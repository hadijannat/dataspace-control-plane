...
# Runs the DCP TCK against the Credential Service actor.
# Sources config/credential-service.env for connection parameters.
...
if [[ ! -f "${ENV_FILE}" ]]; then
    echo "ERROR: ${ENV_FILE} not found. Set DCP_CREDENTIAL_SERVICE_URL." >&2
    exit 1
fi
source "${ENV_FILE}"
