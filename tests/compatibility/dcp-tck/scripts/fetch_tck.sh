#!/usr/bin/env bash
# tests/compatibility/dcp-tck/scripts/fetch_tck.sh
# Downloads the DCP TCK jar from GitHub releases and caches it.
...
if [[ -f "${TCK_JAR}" ]]; then
    echo "DCP TCK jar already cached: ${TCK_JAR}"
else
    DOWNLOAD_URL="${TCK_SOURCE}/releases/download/${TCK_TAG}/dcp-tck-runner-${TCK_TAG}.jar"
    echo "Downloading DCP TCK ${TCK_TAG} from ${DOWNLOAD_URL} ..."
...
echo "DCP TCK jar ready: ${TCK_JAR}"
